...
const updateContaViaTodosBodySchema = z.object({
...
type UpdateContaViaTodosBody = z.infer<typeof updateContaViaTodosBodySchema>

@ApiTags('Conta Corrente')
@ApiBearerAuth()
@Controller('/conta-corrente')
@UseGuards(PermissionsGuard)
export class UpdateContaViaTodosController {
  ...
  @ApiOperation({
    summary: 'Atualizar conta corrente (id no body, via rota /todos)',
  })
  async handle(
    @Body(new ZodValidationPipe(updateContaViaTodosBodySchema))
    body: UpdateContaViaTodosBody,
  ) {
    const { id, ...data } = body
