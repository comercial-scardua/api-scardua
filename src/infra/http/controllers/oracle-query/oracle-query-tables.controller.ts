import { Controller, HttpCode, Post, UseGuards } from '@nestjs/common'
import { ApiBearerAuth, ApiOperation, ApiTags } from '@nestjs/swagger'
import { RequirePermission } from '../../../../auth/decorators/require-permission.decorator'
import { PermissionsGuard } from '../../../../auth/guards/permissions.guard'
import { OracleQueryTablesUseCase } from '../../../../domain/oracle-query/application/use-cases/oracle-query-tables'

@ApiTags('Oracle Query')
@ApiBearerAuth()
@Controller('oracle-query')
@UseGuards(PermissionsGuard)
export class OracleQueryTablesController {
  constructor(private oracleQueryTables: OracleQueryTablesUseCase) {}

  @Post('tables')
  @HttpCode(200)
  @RequirePermission('oracle-query', 'access')
  @ApiOperation({ summary: 'Listar tabelas disponíveis no Oracle' })
  async handle() {
    const result = await this.oracleQueryTables.execute()
    return result.value!
  }
}
