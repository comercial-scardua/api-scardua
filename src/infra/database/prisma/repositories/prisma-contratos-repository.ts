...
} from '../../../../domain/contratos/application/repositories/contratos-repository'
...
export class PrismaContratosRepository implements ContratosRepository {
  ...
  findArquivo(contratoId: number, arquivoId: number) {
    return this.prisma.contrato_arquivos.findFirst({
      where: { id: arquivoId, contrato_id: contratoId },
    })
  }

  create(data: CreateContratoData) {
  ...
  update(id: number, data: UpdateContratoData) {
  ...
  async alternarStatus(id: number) {
    const contrato = await this.prisma.contratos.findUniqueOrThrow({
      where: { id },
    })
  ...
  adicionarArquivo(contratoId: number, arquivo: AddArquivoData) {
