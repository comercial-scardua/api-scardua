...
import {
  ...
  type PatrimoniosRepository,
  type UpdatePatrimonioData,
} from '../../../../domain/patrimonios/application/repositories/patrimonios-repository'
...
export class PrismaPatrimoniosRepository implements PatrimoniosRepository {
  ...
  findAll(showHidden = false) {
    ...
    }) as unknown as ReturnType<PatrimoniosRepository['findAll']>
  }
  ...
  findById(id: number) {
    return this.prisma.patrimonios.findUnique({
      where: { id },
      include: INCLUDE_COMPLETO,
    }) as unknown as ReturnType<PatrimoniosRepository['findById']>
  }
  ...
  create(data: CreatePatrimonioData) {
    ...
    return this.prisma.patrimonios.create({
      data: {
        ...
        nome:
          data.nome ??
          (tipo === 'Linhas Telefônicas' ? 'Linhas Telefônica' : ''),
        tipo,
        locado: locado ?? false,
        franquia: locado ? franquia : undefined,
        data_aquisicao: data.data_aquisicao
          ? new Date(data.data_aquisicao)
          : undefined,
        dataNotaFiscal: data.dataNotaFiscal
          ? new Date(data.dataNotaFiscal)
          : undefined,
        dataGarantia: data.dataGarantia
          ? new Date(data.dataGarantia)
          : undefined,
  ...
  update(id: number, data: UpdatePatrimonioData) {
  ...
  async criarMovimentacao(data: CreateMovimentacaoData): Promise<void> {
  ...
  async toggleOculto(id: number) {
    const atual = await this.prisma.patrimonios.findUniqueOrThrow({
      where: { id },
    })
    return this.prisma.patrimonios.update({
      where: { id },
