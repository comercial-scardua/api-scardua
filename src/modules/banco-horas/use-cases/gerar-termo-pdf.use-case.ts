...
export class GerarTermoPdfUseCase {
  ...
  async execute(colaboradorId: number, dataInicio?: Date, dataFim?: Date): Promise<PassThrough> {
    ...
    // Cabeçalho
    doc.fontSize(18).font('Helvetica-Bold').text('TERMO DE BANCO DE HORAS', { align: 'center' });
    doc.moveDown(0.5);
    ...
    doc.fontSize(10).font('Helvetica');
    const periodoInicio = dataInicio ? dataInicio.toLocaleDateString('pt-BR') : 'Sem filtro';
    const periodoFim = dataFim ? dataFim.toLocaleDateString('pt-BR') : 'Sem filtro';
    ...
    if (registros.length > 0) {
      doc.fontSize(11).font('Helvetica-Bold').text('REGISTROS', { underline: true });
      doc.moveDown(0.3);
      ...
      doc
        ...
        .text('Horas', startX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3], y);

      doc.moveTo(startX, doc.y + 15).lineTo(500, doc.y + 15).stroke();
      doc.moveDown(1);
      ...
      registros.forEach((reg) => {
        ...
        doc.text(entrada, startX + colWidths[0], doc.y - 12, { width: colWidths[1] });
        doc.text(saida, startX + colWidths[0] + colWidths[1], doc.y - 12, {
          width: colWidths[2],
        });
        doc.text(tipo, startX + colWidths[0] + colWidths[1] + colWidths[2], doc.y - 12, {
          width: colWidths[3],
        });
        doc.text(horasFormatadas, startX + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3], doc.y - 12, {
          width: colWidths[4],
        });
    ...
    if (conta) {
      ...
      doc.fontSize(10).font('Helvetica');
      doc.text(`Total Positivo: ${CalculoHorasUtil.formatarHorasDecimal(Number(conta.total_positivo))}`);
      doc.text(`Total Negativo: ${CalculoHorasUtil.formatarHorasDecimal(Number(conta.total_negativo))}`);
      doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .fillColor('#10b981')
        .text(`SALDO: ${CalculoHorasUtil.formatarHorasDecimal(Number(conta.saldo))}`);
    }
