...
export class TestModulesController {
  ...
  async test52Ncms() {
    const data = await this.prisma.ncm.findMany({ take: 52, orderBy: { id: 'asc' } });
    return {
      total: data.length,
      solicitado: 52,
      data,
  ...
  async testBatchSizes() {
    ...
    const results = await Promise.all(
      sizes.map(async (size) => {
        ...
        const data = await this.prisma.ncm.findMany({ take: size, orderBy: { id: 'asc' } });
  ...
  @HttpCode(200)
  @RequirePermission('test-modules', 'access')
  @ApiOperation({ summary: 'Testar sincronização de horas — estatísticas do banco de horas' })
  async testSyncHoras() {
    ...
    return {
      ...
      distribuicaoPorTipo: porTipo.map((t) => ({ tipo: t.tipo, quantidade: t._count.id })),
    };
  }
}
