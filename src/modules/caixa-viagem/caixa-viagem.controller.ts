...
export class CaixaViagemController {
  ...
  async updateTodos(
    ...
    if (!existe)
      throw new NotFoundException(`Caixa de viagem #${id} não encontrado`)
  ...
  async findUltimoCaixa(
    ...
    if (!caixa)
      throw new NotFoundException(
        `Nenhum caixa encontrado para o funcionário #${funcionarioId}`,
      )
    return caixa
  }
  ...
  async generateTermo(@Body('caixaViagemId') caixaViagemId: number) {
    ...
    if (!result)
      throw new NotFoundException(
        `Caixa de viagem #${caixaViagemId} não encontrado`,
      )
    return result
  }
  ...
  criarAdiantamento(
    @Body() dto: CriarAdiantamentoDto,
    @CurrentUser() user: JwtPayload,
  ) {
    return this.repo.criarAdiantamento({
      ...dto,
  ...
  async atualizarAdiantamento(
    ...
    if (!existe)
      throw new NotFoundException(`Adiantamento #${id} não encontrado`)
