...
export type ContaComLancamentos = conta_corrente & {
  ...
  colaborador: { id: number; nome: string | null; sobrenome: string | null } | null;
...
export type ResumoContas = {
  ...
  balanco: number;
  totalContas: number;
  porTipo: { tipo: string; count: number; creditos: number; debitos: number; saldo: number }[];
...
export abstract class ContaCorrenteRepository {
  ...
  abstract create(data: CriarContaCorrenteDto, userId: string): Promise<conta_corrente>;
  abstract update(id: number, data: Partial<CriarContaCorrenteDto>): Promise<conta_corrente>;
