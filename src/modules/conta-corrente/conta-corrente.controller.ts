...
export class ContaCorrenteController {
  ...
  createViaTodos(
    @Body() dto: CriarContaCorrenteDto,
  ...
  async updateViaTodos(
    ...
    if (!existe)
      throw new NotFoundException(`Conta corrente #${body.id} não encontrada`)
  ...
  async findOne(@Param('id', ParseIntPipe) id: number) {
    ...
    if (!conta)
      throw new NotFoundException(`Conta corrente #${id} não encontrada`)
    return conta
  }
