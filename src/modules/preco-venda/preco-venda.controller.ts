...
import type { IncluirPrecoVendaDto, ImportarPrecoVendaDto } from './dto/incluir-preco-venda.dto';
...
export class PrecoVendaController {
  ...
  async importar(@Body() dto: ImportarPrecoVendaDto) {
    const resultados: { produtoId: number; status: string; precoVenda?: number; erro?: string }[] = [];
    ...
    for (const item of dto.itens) {
      ...
      resultados.push({ produtoId: item.produtoId, status: 'atualizado', precoVenda: item.precoVenda });
    }
  ...
  @ApiOperation({ summary: 'Retorna template JSON para importação de preços de venda' })
  template() {
    return {
      descricao: 'Template para importação de preços de venda',
      formato: 'JSON',
      campos: [
        { campo: 'produtoId', tipo: 'number', obrigatorio: true, descricao: 'ID do produto no sistema' },
        { campo: 'precoVenda', tipo: 'number', obrigatorio: true, descricao: 'Preço de venda em reais' },
      ],
      exemplo: {
        itens: [
          { produtoId: 1, precoVenda: 29.99 },
          { produtoId: 2, precoVenda: 49.90 },
        ],
      },
    };
  }
