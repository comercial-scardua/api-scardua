...
export class TabelaDePrecoController {
  ...
  @ApiOperation({ summary: 'Incluir nova tabela de preço (persiste itens como registros em precificador_historico)' })
  async incluir(@Body() dto: IncluirTabelaPrecoDto, @CurrentUser() user: JwtPayload) {
  ...
  @HttpCode(201)
  @RequirePermission('tabela-de-preco', 'edit')
  @ApiOperation({ summary: 'Importar tabela de preços (array de produtos+preços)' })
  async importar(@Body() dto: ImportarTabelaPrecoDto, @CurrentUser() user: JwtPayload) {
    const registros = await Promise.all(
      dto.itens.map((item) =>
        this.prisma.precificador_historico.create({
          data: {
  ...
  @HttpCode(200)
  @RequirePermission('tabela-de-preco', 'edit')
  @ApiOperation({ summary: 'Atualizar tabela existente (insere novos registros como revisão)' })
  async atualizar(@Body() dto: AtualizarTabelaPrecoDto, @CurrentUser() user: JwtPayload) {
  ...
  @ApiOperation({ summary: 'Retorna template de importação de tabela de preços' })
  template() {
    return {
      ...
      exemplo: {
        ...
        itens: [
          { produtoCodigo: 'PROD-001', produtoNome: 'Produto 1', precoVenda: 29.99, precoBase: 20.00 },
          { produtoCodigo: 'PROD-002', produtoNome: 'Produto 2', precoVenda: 49.90 },
        ],
      },
    };
  }
  ...
  @HttpCode(200)
  @RequirePermission('tabela-de-preco', 'access')
  @ApiOperation({ summary: 'Listar histórico de importações de tabela de preços' })
  ...
  async historico(
    ...
    const where = {
      tipoPrecoBase: { in: ['tabela', 'importacao', 'atualizacao'] as string[] },
  ...
  @HttpCode(200)
  @RequirePermission('tabela-de-preco', 'edit')
  @ApiOperation({ summary: 'Limpar histórico de importações (body: { ids?: number[] })' })
  async limparHistorico(@Body() dto: LimparHistoricoDto) {
    if (dto.ids && dto.ids.length > 0) {
      const result = await this.prisma.precificador_historico.deleteMany({
        where: { id: { in: dto.ids } },
      });
      return { deletados: result.count, mensagem: `${result.count} registro(s) removido(s)` };
    }
    ...
    return { deletados: result.count, mensagem: `Histórico limpo: ${result.count} registro(s) removido(s)` };
  }
}
